...
In this way, the AI Blackbird community tried to make the Cat more secure and production-ready.

Moreover, this version can be deployed in a cluster environment. Whilst the original version stored the settings into
JSON files, the AI Blackbird community's version requires a Redis database to store the  settings, the conversation histories, the plugins and so
forth. You can configure the Redis database by environment variables. The [`.env.example`](./.env.example) file is provided as an example.
...
Here, the AI Blackbird community has introduced some new features and improvements, such as:
